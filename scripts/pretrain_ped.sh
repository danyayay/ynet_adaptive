...
for seed in ${list_train_seed[@]}; do
    python train.py --seed $seed --batch_size $batch_size --n_epoch $n_epoch --dataset_path $dataset_path --train_files $train_files --val_files $val_files --val_split $val_split --test_splits $test_splits --train_net $train_net --ckpt_path $ckpt_path --augment --n_round $n_round 
done 
