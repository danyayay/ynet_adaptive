...
for seed in ${list_eval_seed[@]}; do
    for tuned_ckpt in ${list_tuned_ckpt[@]}; do
        python test_embed.py --seed $seed --batch_size $batch_size --dataset_path $dataset_path --val_files $val_files --val_ratio $val_ratio --pretrained_ckpt $pretrained_ckpt --tuned_ckpt $tuned_ckpt --n_leftouts $n_leftouts
    done 
done 
