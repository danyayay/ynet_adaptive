...
pretrained_ckpt=ckpts/Seed_1__Tr_Pedestrian__Val_Pedestrian__ValRatio_0.1__filter_agent_type__train__pretrained.pt
...
#                 python train.py --fine_tune --seed $seed --batch_size $batch_size --n_epoch $n_epoch --dataset_path $dataset_path --train_files $train_files --val_files $val_files --val_ratio $val_ratio --n_leftouts $n_leftouts --train_net $train_net --position $position --pretrained_ckpt $pretrained_ckpt --lr $lr --n_train_batch $n_train_batch
#             done 
#         done 
#     done 
# done
...
for seed in ${list_seed[@]}; do
    for n_train_batch in ${list_n_train_batch[@]}; do
        for lr in ${list_lr[@]}; do
            for train_net in ${list_train_net[@]}; do 
                python train_embed.py --fine_tune --seed $seed --batch_size $batch_size --n_epoch $n_epoch --dataset_path $dataset_path --train_files $train_files --val_files $val_files --val_ratio $val_ratio --n_leftouts $n_leftouts --train_net $train_net --position $position --pretrained_ckpt $pretrained_ckpt --lr $lr --n_train_batch $n_train_batch --swap_semantic
            done 
        done 
    done 
done
...
# list_n_train_batch=(8 16)
# list_lr=(0.001 0.01 0.03)
# list_train_net=(lora_1)
# position="0 1 2 3 4"
# for seed in ${list_seed[@]}; do
#     for train_net in ${list_train_net[@]}; do
