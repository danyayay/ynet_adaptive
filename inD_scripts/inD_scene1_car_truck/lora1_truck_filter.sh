        ...
        for n_train_batch in ${list_n_train_batch[@]}; do 
            for train_net in ${list_train_net[@]}; do 
                for position in "${list_position[@]}"; do
                    python inD_train.py --fine_tune --config_filename 'inD_shortterm_train.yaml' --seed $train_seed --batch_size $batch_size --n_epoch $n_epoch --dataset_path $dataset_path --network $network --load_data $load_data --pretrained_ckpt $pretrained_ckpt --train_net $train_net --position $position --n_fusion $n_fusion --ckpt_path $ckpt_path --n_round $n_round --lr $lr --n_train_batch $n_train_batch --smooth_val --window_size 9
                done 
            done 
        done 
    done 
