        ...
        for lr in ${list_lr[@]}; do
            for train_net in ${list_train_net[@]}; do
                for position in "${list_position[@]}"; do 
                    python train_fusion.py --fine_tune --seed $seed --batch_size $batch_size --n_epoch $n_epoch --dataset_path $dataset_path --train_files $train_files --val_files $val_files --val_split $val_split --n_leftouts $n_leftouts --train_net $train_net --position $position --pretrained_ckpt $pretrained_ckpt --lr $lr --n_train_batch $n_train_batch --ckpt_path $ckpt_path --n_fusion $n_fusion --steps 20
                done 
            done 
        done 
    done 
...
list_position=("scene_motion_fusion" "scene_motion" "fusion")
list_tuned_ckpt=()
for seed in ${list_train_seed[@]}; do
    for n_train in ${list_n_train[@]}; do
        for lr in ${list_lr[@]}; do
            for train_net in ${list_train_net[@]}; do 
                for position in ${list_position[@]}; do
                    list_tuned_ckpt+=("${ckpt_path}/Seed_${seed}__Tr_Biker__Val_Biker__ValRatio_0.1__filter_agent_type_deathCircle_0__${train_net}__Pos_${position}__TrN_${n_train}__lr_${lr}__fusion_${n_fusion}.pt")
                done 
            done 
        done 
    done
...
# python -m pdb train_fusion.py --fine_tune --seed 1 --batch_size 8 --n_epoch 2 --dataset_path filter/agent_type/deathCircle_0/ --train_files Biker.pkl --val_files Biker.pkl --val_split 0.1 --n_leftouts 10 --pretrained_ckpt ckpts/Seed_2__Tr_Pedestrian__Val_Pedestrian__ValRatio_0.1__filter_agent_type__train__fusion_2.pt --lr 0.00005 --n_train_batch 1 --train_net scene_fusion
