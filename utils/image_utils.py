...
def create_dist_mat(size, normalize=True):
    ...
    middle = size // 2
    dist_mat = np.linalg.norm(np.indices(
        [size, size]) - np.array([middle, middle])[:, None, None], axis=0)
    if normalize:
...
def get_patch(template, traj, H, W):
    ...
    patch = [template[y_l:y_u, x_l:x_u]
             for x_l, x_u, y_l, y_u in zip(x_low, x_up, y_low, y_up)]
...
def resize(images, factor, seg_mask=False):
    for key, image in images.items():
        if seg_mask:
            images[key] = cv2.resize(
                image, (0, 0), fx=factor, fy=factor, interpolation=cv2.INTER_NEAREST)
        else:
            images[key] = cv2.resize(
                image, (0, 0), fx=factor, fy=factor, interpolation=cv2.INTER_AREA)
...
def image2world(image_coords, scene, homo_mat, resize):
    ...
    if traj_image2world.dim() == 4:
        traj_image2world = traj_image2world.reshape(
            -1, image_coords.shape[2], 2)
    ...
    traj_image2world = F.pad(input=traj_image2world, pad=(
        0, 1, 0, 0), mode='constant', value=1)
