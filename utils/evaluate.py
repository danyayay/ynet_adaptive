    ...
    with torch.no_grad():
        ...
        for trajectory, df_batch, scene_id in val_loader:
            ...
            for i in range(0, len(trajectory), batch_size):
                ...
                observed_map = get_patch(input_template, observed, H, W)  # TODO: ? list of (height, width)
                observed_map = torch.stack(
                    observed_map).reshape([-1, obs_len, H, W])  # (batch_size, obs_len, height, width)

                gt_future = trajectory[i:i+batch_size, obs_len:].to(device)  # (batch_size, pred_len)
                semantic_image = scene_image.expand(
                    observed_map.shape[0], -1, -1, -1)  # (batch_size, n_class, height, width)

                # Forward pass
                feature_input = torch.cat(
                    [semantic_image, observed_map], dim=1)  # (batch_size, n_class+obs_len, height, width)
                ...
                pred_waypoint_map_sigmoid = model.sigmoid(
                    pred_waypoint_map_sigmoid)  # (batch_size, n_waypoints, height, width)
                        ...
                        for waypoint_num in reversed(range(len(waypoints)-1)):
                            ...
                            if g_num // n_goal == 0:
                                ...
                                waypoint_samples = waypoint_samples.permute(
                                    2, 0, 1, 3)
                                waypoint_samples = waypoint_samples.squeeze(
                                    2).squeeze(0)
                            waypoint_list.append(waypoint_samples)
                ...
                for waypoint in waypoint_samples:
                    ...
                    waypoint_maps_downsampled = [
                        waypoint_map] + waypoint_maps_downsampled  # n_layer=6 list of (batch_size, n_waypoint, height_down, width_down)
                ...
                ade = ((((gt_future - future_samples) / resize_factor) ** 2).sum(dim=3) ** 0.5).mean(dim=2).min(dim=0)[0].cpu().detach().numpy()  # (batch_size, )
                fde = ((((gt_goal - waypoint_samples[:, :, -1:]) / resize_factor) ** 2).sum(dim=3) ** 0.5).min(dim=0)[0][:,0].cpu().detach().numpy()  # (batch_size, )
        ...
        df_out.loc[:, 'metaId'] = np.concatenate(meta_id_list)
        df_out.loc[:, 'sceneId'] = sum(scene_id_list, [])
