...
class SceneDataset(Dataset):
    def __init__(self, data, resize, total_len):
        ...
        self.trajectories, self.meta, self.scene_list = \
			self.split_trajectories_by_scene(data, total_len)
    ...
    def split_trajectories_by_scene(self, data, total_len):
        ...
        for meta_id, meta_df in tqdm(data.groupby('sceneId', as_index=False), desc='Prepare Dataset'):
            trajectories.append(meta_df[['x', 'y']].to_numpy().astype(
                'float32').reshape(-1, total_len, 2))
            meta.append(meta_df)
